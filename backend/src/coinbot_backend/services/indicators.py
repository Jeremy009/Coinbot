...
def macd_indicator(
    ...
    two moving averages of a security's price. It is calculated by subtracting the
    ...
    Args:
        candles: An object with candles data or a DataFrame
        short_period: Number of periods for the short signal (current/recent state)
        long_period: Number of periods for the long signal (global/historic trend)
        signal_period: Number of periods for the EMA on the MACD-line

    Returns:
        DataFrame with additional macd_line and macd_signal columns
    """
    ...
    df = candles.as_dataframe() if not isinstance(candles, pd.DataFrame) else candles.copy()
...
def rsi_indicator(candles: OHLCVCandles, period: int = 14) -> pd.DataFrame:
    """
    Compute the RSI (Relative Strength Index) indicator.

    The RSI is a momentum oscillator indicator between 0 and 100.
    Standard RSI uses a 14 days period or 14 weeks period.

    Traditional interpretation: values of 70 or above indicate that a security is becoming
    overbought or overvalued and may be primed for a trend reversal or corrective pullback
    in price. An RSI reading of 30 or below indicates an oversold or undervalued condition.

    Args:
        candles: An object with candles data
        period: Length of the past period to consider for the RSI computation
...
def mfi_indicator(candles: OHLCVCandles, period: int = 14) -> pd.DataFrame:
    ...
    The MFI is a technical oscillator that uses price and volume data for identifying
    overbought or oversold signals in an asset. It can also be used to spot divergences
    which warn of a trend change in price.

    The oscillator moves between 0 and 100. Generally an MFI reading above 80 is considered
    overbought and an MFI reading below 20 is considered oversold, although levels of 90 and
    10 are also used as thresholds.

    Args:
        candles: An object with candles data
        period: Length of the past period to consider for the MFI computation

    Returns:
        DataFrame with an additional money_flow_index column
    """
